...
export function TenantAssignmentSelect({ id, value, onChange }: TenantAssignmentSelectProps) {
    ...
    useEffect(() => {
        ...
        void tenantService.getForSelect({
            limit: 1,
            selectedTenantId: value,
        }, controller.signal).then((tenants) => {
    ...
    return (
        <div className="w-100">
            <AsyncSelect<TenantOption, false>
                ...
                name="tenantId"
                noOptionsMessage={({ inputValue }) => inputValue ? 'No tenants match this search.' : 'No tenants found.'}
                onChange={handleChange}
                placeholder="Search tenants"
                value={selectValue}
            />
            <div className="form-text">Search by name or email. Results are limited to {tenantResultLimit} tenants.</div>
        </div>
    );
}
