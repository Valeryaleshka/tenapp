...
BEGIN
    ...
    WITH tenant_source AS (
        SELECT
            ...
            'Demo ' || lpad(n::text, 6, '0') AS last_name,
            '+1666' || lpad(n::text, 7, '0') AS phone_number,
            'valeryaleshka.seed.' || lpad(n::text, 6, '0') || '@example.com' AS email
    ...
    WITH property_source AS (
        SELECT
            n,
            'Seed Unit ' || lpad(n::text, 6, '0') AS name,
            CASE (n % 4)
                ...
                ELSE 'Studio'
            END AS type,
            (1000 + n)::text || ' Demo Street, Unit ' || lpad(n::text, 6, '0') AS address,
            (900 + (n % 20) * 75)::numeric(18, 2) AS price,
            1 + (n % 25) AS level,
            date '2026-01-01' + ((n % 365)::int) AS start_date,
            date '2027-01-01' + ((n % 365)::int) AS end_date,
            'valeryaleshka.seed.' || lpad(n::text, 6, '0') || '@example.com' AS tenant_email
        FROM generate_series(1001, 101000) AS n
    )
    ...
    RAISE NOTICE 'Total generated tenant rows for valeryaleshka@gmail.com: %', tenant_count;
    RAISE NOTICE 'Total generated property rows for valeryaleshka@gmail.com: %', property_count;
END $$;
